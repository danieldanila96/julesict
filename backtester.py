...
def load_data(folder_path='tradingdata'):
    ...
    if '1h' in data and 'daily' not in data:
        df_1h = data['1h'].copy()
...
def run_backtest(initial_capital=100000, risk_per_trade=1000):
    ...
    if '5m' not in data or '1h' not in data or 'daily' not in data:
        print("Error: Missing required timeframes (Daily, 1H, 5M) in tradingdata folder.")
        return
